        ...
        nominatimTimeout = setTimeout(async () => {
            try {
                ...
                results.forEach(place => {
                    ...
                    item.style.cssText = `padding:10px 14px;cursor:pointer;font-size:14px;color:#1e293b;border-bottom:1px solid #f1f5f9;line-height:1.4;`;
                    item.innerHTML = `<div style="font-weight:600;">${cityName}</div><div style="font-size:11px;color:#64748b;">${stateName}</div>`;
...
async function handleSubscribe() {
    ...
    if (!email) { showSubscribeStatus('Please enter a valid email address.', 'error'); return; }
...
async function handleUnsubscribe() {
    const email = document.getElementById('subEmail').value.trim();
    if (!email) { showSubscribeStatus('Enter your email first to unsubscribe.', 'error'); return; }
    try {
    ...
    try {
        ...
        container.innerHTML = data.map(a => {
            ...
            <div class="alarm-history-item">
                ...
                <span class="ah-severity" style="background:${color}22;color:${color};border:1px solid ${color}40;">
                    ${a.severity}
                </span>
